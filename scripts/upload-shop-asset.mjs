...
async function run() {
  ...
  if (!localFile) {
    throw new Error("Usage: node scripts/upload-shop-asset.mjs <local-file>");
  }
  ...
  const remoteDir = env.FTP_REMOTE_DIR_SHOP || "public_html/shop";
  const shopRoot = path.join(repoRoot, "public/shop");
  const relativePath = path.relative(shopRoot, absoluteFile).replaceAll("\\", "/");
