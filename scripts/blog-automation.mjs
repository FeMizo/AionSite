...
function buildImagePrompt(post, topic) {
  return [
    ...
    `Exact headline text that MUST be rendered visibly in the left column: "${post.title}".`,
    `Exact badge text that MUST be rendered visibly above the headline: "${post.badgeText}".`,
    "Render a short supporting subtitle below the headline. Do not leave the left text column blank.",
    ...
    "Constraints: no logos, no watermark, no brand marks, no tiny illegible text, no visual elements in the reserved left text-safe column beyond the required headline, badge, and subtitle.",
  ].join(" ");
}
