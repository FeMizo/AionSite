...
export default function EstilosPage() {
  return <main className="style-lab">
    <header className="style-lab-header"><Link href="/" className="lab-brand"><span>A</span><strong>AionSite</strong><small>Style Lab</small></Link><div className="lab-status"><i /> Exploraciones visuales <span>2026</span></div><Link href="/estilos/botones" className="lab-exit">Botones animados <ArrowUpRight size={15} /></Link></header>
    <section className="lab-intro"><div><p className="lab-eyebrow"><Palette size={14} /> Biblioteca de estilos</p><h1>Diseño<br /><em>en movimiento.</em></h1></div><div className="lab-intro-copy"><p>Un espacio personal para explorar las distintas direcciones visuales que estamos construyendo para AionSite.</p><div className="lab-count"><strong>06</strong><span>direcciones<br />disponibles</span></div></div></section>
    <section className="style-grid" aria-label="Estilos disponibles">{styles.map((style, index) => <Link href={style.route} className={`style-card ${style.className}`} key={style.name}><div className="card-top"><span>0{index + 1}</span><span>{style.type}</span><ArrowUpRight className="card-arrow" size={18} /></div><div className="card-preview"><div className="preview-orbit" /><div className="preview-window"><span /><span /><span /></div><strong>{style.className === "luma" ? "L" : style.name.split(" ")[0].slice(0, 1)}</strong></div><div className="card-bottom"><div><h2>{style.name}</h2><p>{style.description}</p></div><div className="swatches">{style.colors.map((color) => <i key={color} style={{ backgroundColor: color }} aria-label={color} />)}</div></div></Link>)}</section>
