...
export default function DesignReferencePage() {
  return (
    <main className="reference-page" style={{
      ...
      <div className="reference-noise" aria-hidden="true" />
      <header className="reference-header">
        <a className="reference-logo" href="#top" aria-label="AionSite inicio"><span>A</span><strong>{design.content.brand}</strong></a>
        ...
        <a className="reference-search" href="#contacto">Contactar <ArrowUpRight size={15} /></a>
        <button className="reference-menu" aria-label="Abrir menú"><Menu size={19} /></button>
      </header>

      <section className="reference-hero" id="top">
        <div className="hero-copy">
          <p className="reference-kicker">{design.content.eyebrow}</p>
          ...
          <a className="reference-button" href="#contacto">{design.content.cta} <ArrowUpRight size={17} /></a>
        </div>
      ...
      <section className="reference-promise" id="consejos">
        <div className="promise-intro"><p className="reference-kicker">{design.content.promise.eyebrow}</p><h2>{design.content.promise.title}</h2><p>{design.content.promise.description}</p></div>
      ...
      <section className="reference-collection" id="coleccion"><div className="collection-heading"><p className="reference-kicker">{design.content.collection.eyebrow}</p><h2>{design.content.collection.title}</h2></div><div className="collection-grid">{design.content.collection.items.map((item, index) => <article key={item} className="collection-card"><div className="collection-image" style={{ backgroundImage: `url(${images[index]})` }} /><div className="collection-meta"><span>{String(index + 1).padStart(2, "0")}</span><h3>{item}</h3><ArrowUpRight size={18} /></div></article>)}</div></section>

      <section className="reference-cta" id="contacto"><div className="cta-mark"><Check size={24} /></div><p>{design.content.closing}</p><a className="reference-button light" href="/">Crear algo extraordinario <ArrowUpRight size={17} /></a></section>
      <footer className="reference-footer"><span>© 2026 {design.content.brand}</span><span>Color · Cuidado · Expresión</span><span>Hecho con intención</span></footer>
    </main>
  );
}
