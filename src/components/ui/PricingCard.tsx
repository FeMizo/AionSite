...
function getPriceParts(rawPrice: string) {
  ...
  return { prefix, amount: working, suffix };
}

export function PricingCard({ plan, whatsappLink }: PricingCardProps) {
  const priceParts = getPriceParts(plan.price);
  const isBasic = /esencial/i.test(plan.name);
  const isAdvanced = /comercio|electr[oó]nico|e-?commerce|tienda/i.test(plan.name);
