...
import { SonarGrid } from "@/src/components/ui/sonar-grid";
    ...
    <section ref={rootRef} onPointerMove={handlePointerMove} className="relative isolate flex min-h-[calc(100vh-20px)] overflow-hidden bg-slate-950 text-white">
      ...
      <Container className="flex min-h-[88dvh] w-full flex-col items-center justify-center pb-10 pt-24 text-center md:pb-12">
        <div className="relative z-10 flex max-w-5xl flex-col items-center">
          ...
          <div className="hero2-reveal mt-9 flex w-full flex-col items-center justify-center gap-3 sm:flex-row"><MotionButton label={data.primaryCTA} classes="w-full shadow-[0_0_44px_-10px_rgba(37,99,235,0.85)] sm:w-auto" icon={<MessageCircleMore size={18} />} onClick={() => window.open(headerData.whatsappLink, "_blank")} /><MotionButton as="a" href="#calculator-section" variant="outline" label={data.secondaryCTA} classes="w-full border-white/20 bg-slate-950/35 backdrop-blur-sm sm:w-auto" icon={<ArrowRight size={16} />} onClick={(event) => { event.preventDefault(); document.getElementById("calculator-section")?.scrollIntoView({ behavior: "smooth" }); }} /></div>
        </div>
